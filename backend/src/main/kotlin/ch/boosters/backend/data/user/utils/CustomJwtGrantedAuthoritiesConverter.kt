package ch.boosters.backend.data.user.utils
...
class CustomJwtGrantedAuthoritiesConverter(private val clientName: String) : Converter<Jwt, Collection<GrantedAuthority>> {
    ...
    private fun extractRealmRoles(mapObject: Any?, finalRoles: MutableCollection<GrantedAuthority>) {
        if (mapObject is Map<*, *>) {
            val rolesMap = mapObject as? Map<String, List<String>>
            rolesMap?.get("roles")?.forEach { role ->
                finalRoles.add(SimpleGrantedAuthority(role))
            }
        }
    }
    ...
    private fun extractClientRoles(jwt: Jwt, finalRoles: MutableCollection<GrantedAuthority>) {
        ...
        resourceAccessMap?.get(clientName)?.get("roles")?.forEach { role ->
            finalRoles.add(SimpleGrantedAuthority(role))
        }
    }
}